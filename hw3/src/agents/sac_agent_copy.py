...
class SoftActorCritic(nn.Module):
    def __init__(
        ...
        if self.auto_tune_temperature:
            ...
            self.log_alpha = None#torch.tensor(np.log(temperature), dtype=torch.float32, requires_grad=True)
            self.alpha_optimizer = None#torch.optim.Adam([self.log_alpha], lr=alpha_learning_rate)
            self.target_entropy = None# -self.action_dim
            # ENDTODO
    ...
    def get_temperature(self) -> float:
        ...
        if self.auto_tune_temperature:
            ...
            return None#self.log_alpha.exp().item()
            # ENDTODO
        else:
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            next_qs = torch.stack([target_critic(next_obs, next_action) for target_critic in self.target_critics])
            # ENDTODO
        ...
        # Predict Q-values
        q_values = torch.stack([critic(obs, action) for critic in self.critics])
 ...
 # replace this with the correct q_values
        ...
        min_q = torch.min(q_values, dim=0).values
        loss = torch.mean(self.get_temperature()*log_prob - min_q) # replace this with the correct loss
        # ENDTODO
    ...
    def update_actor(self, obs: torch.Tensor):
        ...
        # NOTE: Entropy bonus is already included in actor_loss_reparametrize via
        # get_temperature() * log_prob, so no additional term is needed here.
    ...
    def update_alpha(self, log_prob: torch.Tensor):
        ...
        alpha = None#self.log_alpha.exp()
        alpha_loss = None#(alpha * (-log_prob - self.target_entropy)).mean()
        ...
        if self.target_update_period is not None:
            ...
            for critic, target_critic in zip(self.critics, self.target_critics):
                for param, target_param in zip(critic.parameters(), target_critic.parameters()):
                    target_param.data.copy_(
                        target_param.data + self.soft_target_update_rate * (param.data - target_param.data)
                    )
