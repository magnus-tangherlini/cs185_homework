...
class SoftActorCritic(nn.Module):
    def __init__(
        ...
        if self.auto_tune_temperature:
            ...
            self.log_alpha = torch.tensor(np.log(temperature), dtype=torch.float32, requires_grad=True)
    ...
    def actor_loss_reparametrize(self, obs: torch.Tensor):
        ...
        q_values = self.critic(obs, action)#torch.stack([critic(obs, action) for critic in self.critics])
        ...
        loss = -q_values.mean(dim=0).mean()#torch.mean(self.get_temperature()*log_prob - min_q) # replace this with the correct loss
        # ENDTODO
    ...
    def update_actor(self, obs: torch.Tensor):
        ...
        loss -= self.get_temperature() * self.entropy(action_distribution).mean()
        # ENDTODO
