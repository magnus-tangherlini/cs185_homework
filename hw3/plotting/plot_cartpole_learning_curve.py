...
def load_experiment_data(exp_dir: str):
    """Load (env steps, eval return) from a hw3 CartPole experiment directory.
    ...
    try:
        with open(log_file, "r") as f:
            ...
            for row in reader:
                ...
                # Only keep rows where eval return is logged
        ...
        # Simple cleaned name for legend
        clean_name = exp_name.replace("CartPole-v1_", "").split("_sd")[0]
...
def main() -> None:
    """Entry point: find CartPole experiments and plot eval return vs env steps."""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    # Go from hw3/plotting/ → hw3/exp/
    exp_base_dir = os.path.join(script_dir, "..", "exp")
    exp_base_dir = os.path.abspath(exp_base_dir)

    print(f"Looking for CartPole experiments in: {exp_base_dir}")

    if not os.path.exists(exp_base_dir):
        print(f"Experiment directory not found: {exp_base_dir}")
        return

    # Find all CartPole DQN experiment directories
    all_exps = glob.glob(os.path.join(exp_base_dir, "CartPole-v1_*"))
    ...
    print(f"Found {len(experiments)} CartPole experiments with eval data.")

    if not experiments:
        print("No usable CartPole logs found; nothing to plot.")
        return

    # Save plot in the hw3 root directory (parent of exp/)
    output_dir = os.path.dirname(exp_base_dir)
    output_dir = os.path.abspath(output_dir)
    output_path = os.path.join(output_dir, "cartpole_eval_return.png")

    plot_learning_curves(
        experiments,
        title="CartPole-v1 DQN: Eval Return vs Environment Steps",
